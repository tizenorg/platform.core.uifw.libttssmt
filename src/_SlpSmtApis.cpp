...
{
	SLOG(LOG_DEBUG, TTSP_TAG, ">> SLPSMT_StopSynthesis()");
...
int SLPSMT_SynthesizeText(int const iVoiceInfo, char const * pszTextUtf8, void * const pUserParam)
{
	...
	SLOG(LOG_DEBUG, TTSP_TAG, ">> SLPSMT_SynthesizeText()");
	SLOG(LOG_DEBUG, TTSP_TAG, ">>>> iVoiceInfo  : %d", iVoiceInfo );
	SLOG(LOG_DEBUG, TTSP_TAG, ">>>> pszTextUtf8 : %s", pszTextUtf8);
	...
	if (_g.ThreadId < 0)
	{
		...
		if (_g.ThreadId < 0)
		{
			SLOG(LOG_ERROR, TTSP_TAG, ">>>  Fail to create thread");
...
int SLPSMT_GetiVoiceInfoEx(char const *pszLanguage, int const eVoiceType)
{
	...
	for (i=0 ; i<_nVoiceInfos ; i++)
	{
		if ( eVoiceType == _pVoiceInfos[i].eVoiceType  && ! strcmp(pszLanguage ,  _pVoiceInfos[i].pszLanguage)   )
		{
			...
			SLOG(LOG_DEBUG, TTSP_TAG, "%d)####### iResult [%d]",i , iResult);
...
{
	SLOG(LOG_DEBUG, TTSP_TAG, ">>>> SLPSMT_Finalize() called.");
...
//	_g.eSpeechSpeed = TTSP_SPEED_NORMAL;
	...
	SLOG(LOG_DEBUG, TTSP_TAG, ">>>> SLPSMT_Finalize() returns.");
	return TTSP_ERROR_NONE;
}
...
// pszTextUtf8 should not be NULL.
{
	...
	if (! b) { SLOG(LOG_ERROR, TTSP_TAG, ">>>__PushThreadData, out of memory"); }
...
static void * _Synthesize(void* NotUsed)
{
	...
	for ( ; p ; p=_PopThreadData())
	{
		...
		if (_g.pfnCallback)
		{
			...
			SLOG(LOG_DEBUG, TTSP_TAG, ">>>> Thread, _Synthesize(), iVoiceInfo = %d", iVoiceInfo);
			SLOG(LOG_DEBUG, TTSP_TAG, ">>>>>> pszTextUtf8 = %s", pszTextUtf8);

			if (iVoiceInfo != _g.iVoiceInfo)
			{
				r = _ChangeVoice(iVoiceInfo);
				SLOG(LOG_DEBUG, TTSP_TAG, ">>>>>> iVoiceInfo was changed.");
			}
			...
			if (r == SMT_SUCCESS)
			{
				...
				SLOG(LOG_DEBUG, TTSP_TAG, ">>>>>> Set speech-speed");
			...
			SLOG(LOG_DEBUG, TTSP_TAG, ">>>Thread, _Synthesize() done");
		}
...
static int _ChangeVoice(int const iVoiceInfo)
{
	...
	if (r != SMT_SUCCESS) { SLOG(LOG_DEBUG, TTSP_TAG, ">>>  _ChangeVoice() returns %d.", r); }
	else
	{
...
static void _SetSpeechSpeed(void)
{
	SLOG(LOG_DEBUG, TTSP_TAG, ">>>> Set speech speed %d", _g.eSpeechSpeed);
...
static void _CallBack(ttsp_result_event_e eEvent, unsigned int const nPCMs, void * pPCMs, void* pUserParam)
{
	...
	SLOG(LOG_DEBUG, TTSP_TAG, ">>>  callback, pUserParam = 0x%x", (unsigned int)(reinterpret_cast<long>(pUserParam)));
	SLOG(LOG_DEBUG, TTSP_TAG, ">>>  callback, event=");
	...
	if (nPCMs)
	{
		...
		SLOG(LOG_DEBUG, TTSP_TAG, "@@@ saving wave file @@@");
	...
	switch (eEvent)
	{
	case TTSP_RESULT_EVENT_START   : SLOG(LOG_DEBUG, TTSP_TAG, "TTSPE_CBEVENT_SYNTH_START");     break;
	case TTSP_RESULT_EVENT_CONTINUE: SLOG(LOG_DEBUG, TTSP_TAG, "TTSPE_CBEVENT_SYNTH_CONTINUE");  break;
	case TTSP_RESULT_EVENT_FINISH  : SLOG(LOG_DEBUG, TTSP_TAG, "TTSPE_CBEVENT_SYNTH_FINISH");    break;
	case TTSP_RESULT_EVENT_FAIL    : SLOG(LOG_DEBUG, TTSP_TAG, "TTSPE_CBEVENT_SYNTH_FAIL");      break;
	default                          : SLOG(LOG_ERROR, TTSP_TAG, "invalid");                       break;
	}

	if (eEvent==TTSP_RESULT_EVENT_FINISH && ! _bEmptyThreadData())
	{
		SLOG(LOG_DEBUG, TTSP_TAG, ">>> There is another input text.");
		SLOG(LOG_DEBUG, TTSP_TAG, ">>> TTSPE_CBEVENT_SYNTH_FINISH was chanage into TTSPE_CBEVENT_SYNTH_CONTINUE.");
		eEvent = TTSP_RESULT_EVENT_CONTINUE;
	}

	SLOG(LOG_DEBUG, TTSP_TAG, ">>> data size = %d", n);
	SLOG(LOG_DEBUG, TTSP_TAG, ">> >> Here we jump into the callback function.");

	int cbreturn = -1;
	if(_g.pfnCallback != NULL){
		cbreturn = _g.pfnCallback(eEvent, pPCMs, n, TTSP_AUDIO_TYPE_RAW_S16, _g.iSamplingRate, pUserParam);
	}
	SLOG(LOG_DEBUG, TTSP_TAG, ">> >> Here we return from the callback function.");
	SLOG(LOG_DEBUG, TTSP_TAG, ">> >> callback function return value = %d", cbreturn);
	if (-1 == cbreturn)
	{
		SLOG(LOG_DEBUG, TTSP_TAG, ">>> Callback function returns TTS_CALLBACK_HALT.");
		std::lock_guard<std::mutex> lock( _g.bStopMutex );
		_g.bStop         = true;
		_g.bSentenceDone = true;
	}
...
static int _Synthesize_SamsungTTS(char const * const pszTextUtf8, void* pUserParam, int const FramePeriod)
{
	...
	{
		_CleanThreadData();
		SLOG(LOG_DEBUG, TTSP_TAG, ">>>  SMTInputText() returns %d.", r);
	}
	...
	while (r == SMT_SUCCESS)
	{
		...
		else
		{
			...
			default :
				...
				SLOG(LOG_DEBUG, TTSP_TAG, ">>>  SMTSynthesize() returns %d", r);
